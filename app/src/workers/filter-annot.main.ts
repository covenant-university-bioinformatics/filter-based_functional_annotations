...
import { WorkerJob } from '../jobqueue/queue/filter-annot.queue';
import { Job, QueueScheduler, Worker } from 'bullmq';
import { JobStatus, FilterAnnotJobsModel } from '../jobs/models/filter-annot.jobs.model';
import * as path from 'path';
import { FilterAnnotModel } from '../jobs/models/filter-annot.model';
...
const processorFile = path.join(__dirname, 'filter-annot.worker.js');
...
export const createWorkers = async (
  ...
  for (let i = 0; i < config.numWorkers; i++) {
    ...
    worker.on('completed', async (job: Job, returnvalue: any) => {
      ...
      const resultsFile = `${pathToOutputDir}/dropped.txt`;
      const filteredFile = `${pathToOutputDir}/filtered.txt`;

      //update db with result files
      const finishedJob = await FilterAnnotJobsModel.findByIdAndUpdate(
        job.data.jobId,
        {
